...
export const WordList: React.FC<WordListProps> = ({ words, foundWords, onWordClick }) => {
        ...
        <div>
          ...
          <div className="word-list">
            {found.map(word => (
              <div
                key={word.id}
                className="word-item-found cursor-pointer text-center font-medium text-sm"
        ...
        <div>
          ...
          <div className="word-list">
            {remaining.map(word => (
              <div
                key={word.id}
                className="word-item cursor-pointer hover:bg-white hover:bg-opacity-10 text-center font-medium text-sm"
                onClick={() => onWordClick?.(word.id)}
              >
                {word.text}
              </div>
