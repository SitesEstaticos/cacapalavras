...
export const Board = ({ grid, selectedCells, foundWords, words, onCellClick, onCellMouseEnter, onSelectionEnd, }) => {
    ...
    const drawSelectionLine = () => {
        if (selectedCells.length < 2 || !canvasRef.current)
            return;
    ...
    return (<div ref={boardRef} className="relative inline-block" onMouseDown={() => setIsDrawing(true)} onMouseUp={() => {
            setIsDrawing(false);
            onSelectionEnd?.();
        }}>
      <canvas ref={canvasRef} width={totalWidth} height={totalHeight} className="absolute inset-0 pointer-events-none"/>

      <div className="grid gap-1 p-4 card" style={{
            gridTemplateColumns: `repeat(${boardWidth}, minmax(0, 1fr))`,
        }}>
        {grid.map((row, rowIndex) => row.map((cell, colIndex) => (<Cell key={`${rowIndex}-${colIndex}`} letter={cell.letter} isSelected={isCellSelected(rowIndex, colIndex)} isFound={isCellFound(rowIndex, colIndex)} isError={false} onClick={() => onCellClick(rowIndex, colIndex)} onMouseEnter={() => handleMouseEnter(rowIndex, colIndex)}/>)))}
      </div>
    </div>);
};
