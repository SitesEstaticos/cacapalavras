...
export const GamePage = () => {
    ...
    return (<div className="min-h-screen bg-dark p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <Board grid={gameLogic.board} selectedCells={gameLogic.selectedCells} foundWords={gameLogic.foundWords} words={gameLogic.words} onCellClick={handleCellClick} onCellMouseEnter={handleCellMouseEnter} onSelectionEnd={handleSelectionEnd}/>

            {/* Game Status */}
            <div className="mt-6 p-4 card text-center">
              {gameLogic.isGameComplete ? (<p className="text-lg font-bold text-success">✅ Jogo Concluído!</p>) : (<p className="text-lg font-bold text-secondary">
          ...
          <div className="space-y-6">
            {/* HUD */}
            <HUD score={gameLogic.score} time={gameLogic.time} wordsFound={gameLogic.foundWords.length} totalWords={gameLogic.words.length} hintsAvailable={hintsAvailable} difficulty={difficultyLabel} onHintClick={() => setShowHintModal(true)}/>

            {/* Word List */}
            <WordList words={gameLogic.words} foundWords={gameLogic.foundWords} onWordClick={word => console.log('Palavra clicada:', word)}/>
      ...
      <HintModal isOpen={showHintModal} onClose={() => setShowHintModal(false)} onUseHint={handleUseHint} onWatchAd={handleWatchAd} hintsRemaining={hintsAvailable} hint={currentHint || 'Procure uma palavra que faz sentido...'}/>

      <GameOverModal isOpen={showGameOverModal} onClose={() => {
